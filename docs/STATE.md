...
> **Data deste snapshot:** 2026-05-02 — 🎉 **Sprint 6.1 (Tipografia editorial) Done.** MVP feature-complete + Sprint 5 + 5.7 + 6.1. Phases 1+2+3+4 = 100%. Story 6.1 entregue em sessão única: Inter Variable confirmada (wght-only, 224KB de bundle preservado vs +124KB com `opsz`), 10 tokens fontSize semânticos em `tailwind.config.ts` (display-hero/display/h1/h2/h3/kpi-hero/kpi/body/body-sm/label), aplicados em **48 lugares** (14 H1 → display-hero/display, 14 labels → text-label uppercase, KPICard variants → text-kpi-hero/kpi, CardTitle → text-h3), buttons subiram para `font-semibold`. **Camada editorial Sprint 6 estabelecida** — 6.2 (motion), 6.3 (alert hierarchy), 6.4 (DRE refactor), 6.5 (sidebar+FAB) herdam os tokens. Próxima: 6.2 motion ou 6.4 DRE refactor (ambas se beneficiam de tokens já em uso).
...
| **Sprint 6 — Editorial + Motion** | 🟡 **20% (5/25 pts)** | **6.1 Tipografia editorial ✅ Done** (Inter Variable wght-only com paridade de bundle 224KB; 10 tokens fontSize; 48 aplicações; buttons font-semibold). 6.2 motion (framer-motion), 6.3 alert hierarchy 3+5+5, 6.4 DRE refactor visual, 6.5 sidebar 7 itens + FAB contextual — Ready para draft. |
...
**MVP FEATURE-COMPLETE + Sprint 5 + Story 6.1 (Tipografia editorial) fechadas.** Próximo passo: continuar **Sprint 6 — Editorial + Motion** com 6.2 (motion / framer-motion) ou 6.4 (DRE refactor visual). Ambas se beneficiam dos tokens da 6.1 já em produção.
