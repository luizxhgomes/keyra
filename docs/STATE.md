...
> **Data deste snapshot:** 2026-05-02 — 🎉 **Sprint 6.0 + 6.1 + 6.3 + 6.4 Done — DS consolidada com gates anti-regressão permanentes.** MVP feature-complete + Sprint 5 + 5.7 + 6.0 + 6.1 + 6.3 + 6.4. Phases 1+2+3+4 = 100%. **Story 6.0 (Higienização pré-motion + gates anti-regressão)** fechada após auditoria `@baziotti` (score 7.6/10) detectar 4 issues HIGH escapados das stories 6.1/6.3/6.4. Análise de causa-raiz feita pelo orquestrador identificou raiz comum: falta de gates cross-cutting. 16 arquivos modificados: AC1 reescreveu 3 alertas em valores absolutos (princípio CON-UX-01 violado desde 4.9 — agora corrigido); AC2 sweep cromático `text-emerald-700` → `text-lucro` em 14 lugares (badge "Top" out-of-scope preservado); AC3 `min-h-[44px]` AA em action e secondaryAction do AlertCard; AC4 banner truncamento agenda → `<AlertCard severity="warning">` consistente; AC5 3 cosméticos; AC6 comentários JSDoc registrando decisões da idealizadora (manter coluna `% sobre receita` ratio analítico + ordem do dashboard com KPIs hero primeiro). **AC7 INSTAURA Phase 2.5 em `.claude/rules/story-lifecycle.md` como regra permanente** com 4 gates: G1 (princípios inegociáveis), G2 (inventário de tokens), G3 (touch target AA), G4 (fonte única real). Aplicada retroativamente na própria 6.0 (eat your own dog food). **Camada DS 100% pronta para 6.2 (motion)** sem furos cromáticos, sem violação de princípio inegociável, sem touch targets abaixo de AA. Phase 3.5: Financial Gate APROVADO; Compliance/Growth WAIVED.
...
| **Sprint 6 — Editorial + Motion** | 🟡 **80% (20/25 pts)** | **6.0 ✅ Done** (higienização pré-motion: 4 HIGH do baziotti + Phase 2.5 Anti-Regression Gates instaurada) + **6.1 ✅ Done** (Inter Variable + 10 tokens fontSize) + **6.3 ✅ Done** (alert hierarchy 3+5+5 + dismiss localStorage SSR-safe) + **6.4 ✅ Done** (refactor visual DRE + 10 H2s tokenizados). **Camada DS 100% consolidada com gates permanentes contra regressões.** Restam: 6.2 motion (framer-motion), 6.5 sidebar 7 itens + FAB contextual. |
...
**Stories 6.0, 6.1, 6.3 e 6.4 da Sprint 6 fechadas — DS 100% consolidada + Phase 2.5 Anti-Regression Gates instaurada como regra permanente.** Próximo passo: **6.2 (Motion / framer-motion)** entra sobre uma camada de design system **sem furos** — todos os 4 issues HIGH do baziotti corrigidos, gates G1-G4 prevenindo regressões futuras. Após 6.2, fechar Sprint 6 com 6.5 (sidebar 7 itens + FAB contextual).
